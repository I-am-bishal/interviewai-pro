...
const STATS = [
  { label: 'Total Interviews', value: '24', change: '+4 this week', icon: Target, up: true, color: '#3b82f6' },
  { label: 'Average Score',    value: '82%', change: '+6% improved', icon: TrendingUp, up: true, color: '#10b981' },
  { label: 'Current Streak',  value: '🔥 7d', change: 'Personal best!', icon: Zap, up: true, color: '#f59e0b' },
  { label: 'Badges Earned',   value: '12',  change: '2 new unlocked', icon: Award, up: true, color: '#7c5bf0' },
];
...
const Dashboard = () => {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <motion.div variants={itemVariants} className="flex items-center justify-between mb-7 relative z-10">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Ready for today's interview practice?</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7 relative z-10">
        {STATS.map(({ label, value, change, icon: Icon, up, color }) => (
          <motion.div key={label} variants={itemVariants}>
            <Card className="!p-4 group hover:-translate-y-0.5 transition-all duration-300">
              ...
              <div className="font-heading text-2xl font-extrabold mb-1 tracking-tight">
                <AnimatedCounter value={value} />
              </div>
              <div className={`text-[11.5px] font-medium ${up ? 'text-success' : 'text-danger'}`}>{change}</div>
            </Card>
          </motion.div>
        ))}
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {INTERVIEW_MODES.map((mode) => (
              <motion.div key={mode.id} variants={itemVariants}>
                <Card hover onClick={() => startInterview(mode.id)} className="!p-5 group transition-all duration-300">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${mode.gradient} border border-border/30 flex items-center justify-center text-xl mb-3`}>
                    {mode.emoji}
                  </div>
                  <div className="font-heading font-bold text-[15px] mb-1.5 tracking-tight">{mode.label}</div>
                  ...
                  <div className="flex items-center justify-end mt-3 text-accent text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="space-y-4">
          ...
          <motion.div variants={itemVariants}>
            ...
            <Card className="animated-border bg-transparent border-transparent">
              ...
              <div className="font-heading font-bold mb-1 tracking-tight">Design a Distributed Cache</div>
              <div className="text-[12.5px] text-slate-500 mb-4">System Design · 30 min · 150 XP</div>
            ...
            <Card className="!p-4 space-y-3">
              {[
                ...
                <div key={topic} className="flex items-center gap-3">
                  ...
                  <span className="flex-1 text-[13px] text-slate-600 truncate">{topic}</span>
                  <div className="flex items-center gap-2">
                    <span className="font-heading font-bold text-accent text-sm">{score}%</span>
                    <ProgressBar value={score} className="w-14" />
                  </div>
                </div>
              ))}
              <button
                onClick={() => navigate('/analytics')}
                className="w-full text-center text-[12px] text-accent/60 hover:text-accent transition-colors pt-1 font-medium"
              >
                View all →
              </button>
            </Card>
