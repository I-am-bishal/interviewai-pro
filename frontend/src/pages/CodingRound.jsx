...
const CodingRound = () => {
  ...
  if (!problem) return (
    <div className="p-6 text-center text-white/40">No problems available</div>
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`font-mono text-sm px-3 py-1.5 rounded-lg border ${
            isDanger
              ...
              : 'text-white/50 border-border bg-bg-3'
          }`}>
            {formatted}
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[42%] border-r border-border overflow-y-auto flex-shrink-0">
          <div className="p-5">
            ...
            <p className="text-[13.5px] text-white/70 leading-relaxed mb-5">{problem.description}</p>
            ...
            <ul className="space-y-1 mb-5">
              {problem.constraints.map((c, i) => (
                <li key={i} className="text-[12.5px] text-white/50 font-mono flex items-start gap-2">
                    ...
                    >
                      <div className="mt-3 space-y-2">
                        {problem.hints.map((h, i) => (
                          <div key={i} className="bg-warning/8 border border-warning/20 rounded-lg px-3 py-2 text-[12.5px] text-white/60">
                            💡 {h}
                          </div>
                        ))}
                      </div>
                  ...
                  {output.results.map((r, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {r.timeMs && <span className="text-white/40">{r.timeMs}ms</span>}
                      </div>
                      <div className="text-white/50">Input: {r.input}</div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-2.5 bg-bg-2 border-b border-border flex-shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-white/30 ml-1 font-mono">solution</span>
            </div>
            <select
              value={language}
              onChange={(e) => setLanguage(e.target.value)}
              className="bg-bg-3 border border-border text-white text-xs px-2.5 py-1.5 rounded-lg outline-none focus:border-accent transition-colors cursor-pointer"
            >
              {LANGUAGES.map((l) => (
            ...
            <div className="p-4">
              <div className="flex items-center gap-6 flex-wrap">
                ...
                <div className="flex gap-4 text-sm">
                  <div>
                    <div className="text-white/40 text-xs mb-1">Time Complexity</div>
                    <div className="font-mono font-semibold text-info">{aiEval.timeComplexity}</div>
                  </div>
                  <div>
                    <div className="text-white/40 text-xs mb-1">Space Complexity</div>
                    <div className="font-mono font-semibold text-info">{aiEval.spaceComplexity}</div>
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-white/40 mb-1">AI Summary</div>
                  <div className="text-[13px] text-white/70 leading-relaxed">{aiEval.summary}</div>
